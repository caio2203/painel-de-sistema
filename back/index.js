...
app.post('/pacientes', async (req, res) => {
  try {
    ...
    const resultado = await runQuery(db, 'INSERT INTO pacientes (nome, status, medico_id, data_criacao) VALUES (?, ?, ?, ?)', [nome, status, medico_id, dataAtual]);
    const insertId = DB_TYPE === 'sqlite' ? resultado.lastID : resultado.insertId;
    if (DB_TYPE === 'sqlite') await db.close();
    res.status(201).json({ id: insertId, nome, status, medico_id, data_criacao: dataAtual });
  } catch (erro) {
    res.status(500).json({ mensagem: 'Erro ao cadastrar paciente', erro: erro.message });
  }
});
